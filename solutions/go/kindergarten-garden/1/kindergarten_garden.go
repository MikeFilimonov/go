...
func NewGarden(diagram string, children []string) (*Garden, error) {
	...
	plantRows := strings.Fields(diagram)

	if len(plantRows) != 2 {
		return nil, fmt.Errorf("expected only two rows of plants")
	}
	...
	if !sort.StringsAreSorted(children) {
		for k, v := range plantRows {
			plantRows[k] = recomposeThePlants(v, children)
		}

	}

	for k, kidName := range children {
